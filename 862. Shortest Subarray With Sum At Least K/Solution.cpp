...
public:
    int shortestSubarray(vector<int>& nums, int K) {
        ...
        while (j < N) {
            ...
            // If the cumulative sum from the start to j is already >= K, update
            // result
            ...
            while (!deq.empty() &&
                ...
                result = min(
                    result,
                    j - deq.front()); // Calculate the length of the subarray
                deq.pop_front();      // Remove the front index from the deque
            }

            // Maintain the monotonic property of the deque (increasing order of
            // cumulative sums)
            while (!deq.empty() &&
